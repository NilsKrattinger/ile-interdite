...
public class InterruptionController {
    ...
    public void initDiscard(Adventurer adventurer, ArrayList<Card> cards) {
        ArrayList<String> cardNamesToDiscard;
        if (cards.size() == Hand.NB_MAX_CARDS) {
            cardNamesToDiscard = discardView.getCardsToDiscard(cards,1);
        } else {
            cardNamesToDiscard = discardView.getCardsToDiscard(cards,cards.size() - Hand.NB_MAX_CARDS);
        }
